...
char ssid[] = "Romagnolo 2.4G";
char password[] = "melzinha123";
...
void sendSerialKey () {
  ...
  if(devMode) {
    ...
    wsclient.send(onOpenMessage + marker + serialKey);
  }  
}
...
String getServerUri () {
  if (devMode) {
    return String(websocketServerHostLocal) + ":" + String(websocketServerPort);
  } else {
    return String(websocketServerHost);
  }
}
