...
void sendSerialKey () {
  ...
  String marker = "$S";
  if(ENABLE_DEV_MODE) {
    wsclient.send(marker + serialKeyDev);
  } else {
    wsclient.send(marker + serialKeyProd);
  }  
}
...
void loop() {
  ...
  if (wateringRoutineMode) {
    ...
    if (hours >= settings[4] && hours <= settings[5]) {
      if ((millis() - beginWateringRoutineTimer > settings[7]) && beginRelayTimer == 0) { // check if the interval has passed;
        Serial.print("Auto watering activated");
  ...
  float internalHumidity = sht20.readHumidity();
  float internalTemperature = sht20.readTemperature();
...
//  lcd.setCursor(2 , 1);
  ...
  if (millis() - beginSendMeasureTimer > settings[3]) {
    ...
    ThingSpeak.setField(1, internalHumidity);
    ThingSpeak.setField(2, internalTemperature);
